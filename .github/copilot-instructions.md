...
- Persistence is split by concern: PostgreSQL (Bun) for durable entities (`users`, `user_tokens`) and Redis for session tokens + auth rate-limiting state. Auth session validity is enforced by matching JWTs against Redis-stored active tokens.
