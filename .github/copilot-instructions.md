...
- Persistence is split by concern: PostgreSQL (GORM) for durable entities (`users`, `password_reset_tokens`) and Redis for session tokens. Auth session validity is enforced by matching JWTs against Redis-stored active tokens.
