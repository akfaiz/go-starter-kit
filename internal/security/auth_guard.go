...
func NewAuthGuard(cfg config.Config, rdb *redis.Client) domain.AuthGuard {
	return &authGuard{cfg: cfg.RateLimit, rdc: cfg.Redis, rdb: rdb}
}

func (g *authGuard) CheckLogin(ctx context.Context, ip, email string) (*domain.RateLimitResult, error) {
	email = strings.ToLower(strings.TrimSpace(email))
	if email == "" {
		return &domain.RateLimitResult{}, nil
	}
	...
	if ttl > 0 {
		return &domain.RateLimitResult{Limited: true, RetryAfter: int(ttl.Seconds()), LockoutActive: true}, nil
	}
	return g.checkWindowLimit(ctx, g.key("auth:login:ip", ip), g.cfg.LoginAttempts, g.cfg.LoginWindow)
}

func (g *authGuard) OnLoginFailure(ctx context.Context, ip, email string) (*domain.RateLimitResult, error) {
	email = strings.ToLower(strings.TrimSpace(email))
	if email == "" {
		return &domain.RateLimitResult{}, nil
	}
	...
	if attemptsCmd.Val() >= g.cfg.LoginLockoutThreshold {
		...
		return &domain.RateLimitResult{Limited: true, RetryAfter: int(g.cfg.LoginLockoutDuration.Seconds()), LockoutActive: true}, nil
	}
...
func (g *authGuard) CheckRefresh(ctx context.Context, ip string) (*domain.RateLimitResult, error) {
	return g.checkWindowLimit(ctx, g.key("auth:refresh:ip", ip), g.cfg.RefreshAttemptsPerIP, g.cfg.RefreshWindow)
}

func (g *authGuard) checkWindowLimit(ctx context.Context, key string, limit int64, window time.Duration) (*domain.RateLimitResult, error) {
	pipe := g.rdb.TxPipeline()
	...
	if count > limit {
		return &domain.RateLimitResult{Limited: true, RetryAfter: int(ttl.Seconds())}, nil
	}
	return &domain.RateLimitResult{}, nil
}
